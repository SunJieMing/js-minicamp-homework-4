...
function sumArray(numbers, cb) {
	...
	cb(numbers.reduce(function(totes, num){totes += num; return totes;}));
}

function forEach(arr, cb) {
	//iterate over arr and pass its values to cb one by one
	//hint: you will be invoking cb multiple times (once for each value in the array)
	arr.forEach(function(element){cb(element);});
}
...
function map(arr, cb) {
	...
	var mapArray = [];
	arr.forEach(function(element){mapArray.push(cb(element));});
	return mapArray;
}
